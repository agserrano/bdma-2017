...
# Laboratory 2: Ridge, LASSO and standard regression
####################################################
...
t <- as.numeric(bodyfat.data[,4]) # target
x <- as.matrix(bodyfat.data[,1:3]) # predictors

model.lasso <- lars(x, t, type="lasso")

# shown are the different cut points for the value of the coefficients
plot(model.lasso)
...
# we decide to cut on lambda=0.25915614 (this eliminates 'midarm')
abline(v=lambda.lasso[3], lty=2)
abline(h=0, lty=2)

# these are the corresponding coefficients
