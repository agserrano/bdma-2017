...
public class SelectionMapReduce extends JobMapReduce {
	...
	public static class SelectionMapper extends Mapper<Text, Text, Text, Text> {
		...
		public void map(Text key, Text value, Context context) throws IOException, InterruptedException {
			...
			String selectionValueSet = context.getConfiguration().getStrings("selectionValueSet")[0];

			String[] arrayValues = value.toString().split(",");
			String selectionValue = Utils.getAttribute(arrayValues, selection);			
			
			if (selectionValue.equals(selectionValueSet)) {
	...
	public boolean run() throws IOException, ClassNotFoundException, InterruptedException {
	    ...
	    job.getConfiguration().setStrings("selectionValueSet", "type_1");
